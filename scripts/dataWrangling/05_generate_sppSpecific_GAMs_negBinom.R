...
save_gam_csv <- function(g){
  ...
  if(nrow(filter(g, fit > 0))){
    ...
    fp_csvs <- file.path('data/sppSpecific_gamOutputsCSVs_negBinom/')
    fp_figures <- file.path('figOutputs/gamsPoisson_negBinom/')
...
gam_pipeline <- function(binomial){
  ...
  ggsave(filename = paste0("figOutputs/gamsPoisson_negBinom/", bw, ".png"), plot = cp, 
         width = 7, height = 7)
...
lapply(X = spp_list, FUN = gam_pipeline)
